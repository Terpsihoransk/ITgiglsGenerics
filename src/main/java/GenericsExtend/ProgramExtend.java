package GenericsExtend;

public class ProgramExtend {
...
class Transaction<T extends Account> { // компилятор распознает объекты типа Т как объекты типа Account =>
                                        // мы можем вызывать у объектов типа Т методы класса Account
